(function () {
  ...
  function initRabbitScene() {
    ...
    function sphere(name, scale, position, material, segments) {
      var mesh = new THREE.Mesh(new THREE.SphereGeometry(1, segments || 24, segments || 24), material || white);
      mesh.name = name;
    ...
    function wing(side) {
      ...
      for (var i = 0; i < 6; i += 1) {
        var feather = new THREE.Mesh(new THREE.SphereGeometry(1, 18, 18), pearl);
    ...
    if (host) {
      ...
      host.addEventListener("pointermove", function (event) {
        ...
        if (isCover) {
          body.style.setProperty("--hero-x", pointerX.toFixed(3));
          body.style.setProperty("--hero-y", pointerY.toFixed(3));
          var hero = qs(".cover-hero");
          if (hero) {
    ...
    if ("IntersectionObserver" in window && host) {
      var sceneObserver = new IntersectionObserver(function (entries) {
        sceneActive = entries.some(function (entry) {
    ...
    function render(now) {
      ...
      if (!reduceMotion && now && now - lastFrame < 34) {
